...
const fetchRsvp = firebase => () =>
  firebase.firestore
    .collection("rsvps")
    .doc(firebase.auth.currentUser && firebase.auth.currentUser.uid);
...
const updateRsvp = firebase => updatedRsvpInfo => {
  if (
    ...
    !updatedRsvpInfo.submitted &&
    firebase.auth.currentUser.accepted
  )
    firebase.firestore
      .collection("rsvps")
  ...
  if (firebase.auth.currentUser && firebase.auth.currentUser.accepted)
    firebase.firestore
      ...
      .update({
        ...updatedRsvpInfo,
        submitted: true,
        submittedAt: firebase.getTimestamp(new Date())
      });
};
...
const RsvpViewContainer = ({
  ...
  if (value) {
    if (
      value.exists &&
      (rsvpState === "FETCHING" ||
        (value.data().submitted && rsvpState === "SUBMITTING"))
    ) {
